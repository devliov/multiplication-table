 ...
 */
function initialize() {
  const titleElement = buildH1(
    `Tabuada de multiplicação (<span class="title_symbol">x</span>)`,
    "title",
    "title"
  );
...
function handleInput(event) {
  ...
  numericValues.forEach((numericValue) => {
    ...
    const containerElement = prepareContainer(
...
function prepareContainer(containerResultElement, inidicateNumber) {
  ...
  const _indicateNumber = buildDiv({
    className: "indicateNumber",
    id: `indicateNumber-${inidicateNumber}`,
  });
